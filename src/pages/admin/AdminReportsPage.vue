...
// Grief review modal — replaces window.prompt/confirm per CLAUDE.md.
const reviewModal = ref<{ open: boolean; report: GriefReport | null; action: 'approve' | 'reject' }>({
  open: false, report: null, action: 'approve',
})
...
async function load() {
  ...
  error.value = null
  try {
    if (tab.value === 'toxic')      toxicRows.value  = await api.getAdminToxicReports({ limit: 200 })
...
function openReview(report: GriefReport, action: 'approve' | 'reject') {
  reviewModal.value = { open: true, report, action }
  reviewNote.value = ''
...
async function submitReview() {
  ...
  try {
    ...
    if (reviewModal.value.action === 'approve') {
      await api.approveGriefReport(r.id, note)
    } else {
      await api.rejectGriefReport(r.id, note)
    }
    closeReview()
    await load()
  } catch (e: any) {
  ...
  <div class="p-4 md:p-8 md:px-10 flex flex-col gap-4 md:gap-6 max-w-[var(--admin-content-max,1200px)] w-full">
    ...
    <template v-else-if="tab === 'toxic'">
      ...
      <div v-else class="flex flex-col gap-2">
        <div v-for="r in toxicRows" :key="r.id" class="card p-3 flex items-center gap-3 flex-wrap">
          <div class="flex items-center gap-2 min-w-0 w-[280px]">
            <Flag class="w-4 h-4 text-amber-400 shrink-0" />
            <img v-if="r.reported_avatar" :src="r.reported_avatar" class="w-7 h-7 rounded-full" />
          ...
          <div class="flex items-center gap-1.5 min-w-0">
            <img v-if="r.reporter_avatar" :src="r.reporter_avatar" class="w-6 h-6 rounded-full" />
            <div v-else class="w-6 h-6 rounded-full bg-accent" />
            <span class="text-sm truncate">{{ r.reporter_display_name }}</span>
          </div>
          <span v-if="r.comment" class="text-xs italic text-muted-foreground truncate max-w-[280px]" :title="r.comment">"{{ r.comment }}"</span>
    ...
    <ModalOverlay :show="reviewModal.open" @close="closeReview">
      <div class="px-6 py-5 border-b border-border">
        ...
        <p v-if="reviewModal.report" class="text-xs text-muted-foreground mt-0.5">
          {{ t('inhouseReportAgainst', { name: reviewModal.report.reported_display_name }) }}
        </p>
      </div>
      <div class="px-6 py-5 flex flex-col gap-3">
        <p v-if="reviewModal.action === 'approve'" class="text-xs text-muted-foreground">
          {{ t('griefApproveExplain') }}
        </p>
