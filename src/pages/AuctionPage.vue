...
import { Gavel, Pause, Play, XCircle, Zap, History, Wallet, Users as UsersIcon, AlertCircle, CheckCircle, Circle } from 'lucide-vue-next'
...
function updateTimer() {
  if (store.auction.bidTimerEnd > 0) {
    const remaining = Math.max(0, Math.ceil((store.auction.bidTimerEnd - Date.now()) / 1000))
...
function placeBid(increment: number) {
  ...
  setTimeout(() => { bidCooldown.value = false }, 2000)
}
...
const captainRosters = computed(() =>
  store.captains.value.map(c => ({
    ...c,
    players: store.players.value.filter(p => p.drafted && p.drafted_by === c.id),
  }))
)
...
<template>
  <div class="p-6 px-8 flex flex-col gap-5">
    <!-- Toast notifications (fixed top-right) -->
    <div class="fixed top-4 right-4 z-50 flex flex-col gap-2 w-[360px] pointer-events-none">
    ...
    <div class="flex items-center justify-between">
      <div>
        <h1 class="text-2xl font-semibold text-foreground">Live Auction</h1>
      ...
      <div class="flex items-center gap-3" v-if="(isActive || isPaused) && store.isAdmin.value">
        ...
        <div class="divide-y divide-border">
          <div v-for="captain in store.captains.value" :key="captain.id" class="flex items-center justify-between px-4 py-3">
            <div class="flex items-center gap-3">
              <div class="w-8 h-8 rounded-full flex items-center justify-center text-xs font-semibold" :class="store.onlineCaptainIds.value.includes(captain.id) ? 'bg-green-500 text-white' : 'bg-secondary text-secondary-foreground'">
    ...
    <div v-if="isActive || isPaused" class="flex gap-5">
      ...
      <div class="flex-1 flex flex-col gap-5 min-w-0">
        ...
        <div v-if="isNominating" class="card">
          <div class="flex items-center gap-2 px-4 py-3 border-b border-border">
            ...
            <span class="text-sm font-semibold text-foreground">Nominate a Player</span>
            <span class="badge-info ml-2">NOMINATING</span>
          </div>
          <div class="p-4">
            <p class="text-sm text-muted-foreground mb-4" v-if="store.auction.nominator">
              {{ store.auction.nominator.name }}'s turn to nominate.
              ...
              <table class="w-full text-sm">
                <thead>
                  <tr class="border-b border-border bg-accent/50">
                    ...
                    <th class="text-left px-4 py-2 font-medium text-muted-foreground w-[120px]" v-if="isNominator || store.isAdmin.value">START BID</th>
                ...
                <tbody>
                  <tr v-for="player in store.availablePlayers.value" :key="player.id" class="border-b border-border hover:bg-accent/30 transition-colors">
                    ...
                    <td class="px-4 py-2.5">
                      <span v-for="role in player.roles" :key="role" class="inline-flex items-center rounded-full px-2 py-0.5 text-[10px] font-medium mr-1" :class="roleColors[role]">{{ role }}</span>
                    </td>
                    <td class="px-4 py-2.5 font-mono text-foreground">{{ player.mmr.toLocaleString() }}</td>
                    <td class="px-4 py-2.5" v-if="isNominator || store.isAdmin.value">
                      <input
                        type="number"
                    ...
                    <td class="px-4 py-2.5 text-right">
                      <button v-if="isNominator || store.isAdmin.value" class="btn-primary text-xs py-1.5 px-3" @click="nominatePlayer(player.id)">Nominate</button>
                    </td>
                  </tr>
                </tbody>
              </table>
              ...
              <div class="p-4 flex flex-col gap-4" v-if="store.auction.nominatedPlayer">
                <div class="flex items-center gap-4">
                  <div class="w-16 h-16 rounded-full bg-secondary flex items-center justify-center text-xl font-bold text-secondary-foreground">
                    {{ store.auction.nominatedPlayer.name.slice(0, 2) }}
                  </div>
                  <div>
                    <p class="text-lg font-semibold text-foreground">{{ store.auction.nominatedPlayer.name }}</p>
                    <p class="text-sm text-muted-foreground">
                      {{ store.auction.nominatedPlayer.roles.join(', ') }} &bull; MMR: {{ store.auction.nominatedPlayer.mmr.toLocaleString() }}
                    </p>
                    <p class="text-xs text-muted-foreground mt-0.5" v-if="store.auction.nominator">
                ...
                <div class="flex gap-4">
                  <div class="flex-1 rounded border border-border p-3 text-center">
                    <p class="text-xs text-muted-foreground uppercase tracking-wider">Current Bid</p>
                    <p class="text-2xl font-bold text-primary font-mono mt-1">{{ store.auction.currentBid }}g</p>
                    <p class="text-xs text-muted-foreground" v-if="store.auction.currentBidder">by {{ store.auction.currentBidder.name }}</p>
                  </div>
                  <div class="flex-1 rounded border border-border p-3 text-center">
                    <p class="text-xs text-muted-foreground uppercase tracking-wider">Time Left</p>
                    <p class="text-2xl font-bold font-mono mt-1" :class="timeLeft <= 5 ? 'text-destructive' : 'text-foreground'">{{ timerDisplay }}</p>
                    <p class="text-xs text-muted-foreground">seconds remaining</p>
                  </div>
                  <div class="flex-1 rounded border border-border p-3 text-center">
                    <p class="text-xs text-muted-foreground uppercase tracking-wider">Bids Placed</p>
                    <p class="text-2xl font-bold text-foreground font-mono mt-1">{{ store.auction.bidHistory.length }}</p>
                    <p class="text-xs text-muted-foreground">this round</p>
                  </div>
                </div>

                <div class="flex gap-3" v-if="!isPaused">
                  <template v-if="store.currentCaptain.value">
                    <button class="btn-outline flex-1 justify-center" :disabled="bidCooldown" @click="placeBid(5)">+5g</button>
                    <button class="btn-outline flex-1 justify-center" :disabled="bidCooldown" @click="placeBid(10)">+10g</button>
                    <button class="btn-outline flex-1 justify-center" :disabled="bidCooldown" @click="placeBid(25)">+25g</button>
                    <button class="btn-primary flex-1 justify-center" :disabled="bidCooldown" @click="placeBid(store.settings.bidIncrement)">
            ...
            <table class="w-full text-sm">
              ...
              <tbody>
                <tr v-for="player in store.availablePlayers.value" :key="player.id" class="border-b border-border">
                  ...
                  <td class="px-4 py-2 text-right font-mono text-foreground">{{ player.mmr.toLocaleString() }}</td>
                </tr>
              </tbody>
            </table>
          </div>
      ...
      <div class="w-[300px] flex-shrink-0 flex flex-col gap-3 overflow-y-auto max-h-[calc(100vh-140px)]">
        <div v-for="roster in captainRosters" :key="roster.id" class="card">
          <div class="flex items-center justify-between px-3 py-2.5 border-b border-border">
            <div class="flex items-center gap-2">
              <div class="w-7 h-7 rounded-full flex items-center justify-center text-[10px] font-semibold" :class="store.onlineCaptainIds.value.includes(roster.id) ? 'bg-green-500 text-white' : 'bg-secondary text-secondary-foreground'">{{ roster.name.charAt(0) }}</div>
              <div>
          ...
          <div v-if="roster.players.length > 0" class="divide-y divide-border">
            <div v-for="player in roster.players" :key="player.id" class="flex items-center justify-between px-3 py-2">
              <div class="flex items-center gap-2 min-w-0">
                ...
                <span v-for="role in player.roles" :key="role" class="inline-flex items-center rounded-full px-1.5 py-0 text-[9px] font-medium flex-shrink-0" :class="roleColors[role]">{{ role }}</span>
              </div>
